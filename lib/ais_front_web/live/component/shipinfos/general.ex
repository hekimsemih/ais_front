defmodule AisFrontWeb.Live.Component.Shipinfos.General do
  ...
  def render(%{shipinfos: _shipinfos} = assigns) do
    ...
    <article id="shipinfos-jump">
      <h4>Jump to ship:</h4>
      <ul>
        <li class="jump-button ship-icon <%= @shipinfos.type_short_name %>" title="large" onclick="jumpto(<%= FullShipinfos.str_coordinates(@shipinfos) %>, 6)">
          <svg class="large-jump">
            <%= live_component @socket, Svg, image: "boat.svg" %>
          </svg>
        </li>
        <li class="jump-button ship-icon <%= @shipinfos.type_short_name %>" title="medium" onclick="jumpto(<%= FullShipinfos.str_coordinates(@shipinfos) %>, 12)">
          <svg class="medium-jump">
            <%= live_component @socket, Svg, image: "boat.svg" %>
          </svg>
        </li>
        <li class="jump-button ship-icon <%= @shipinfos.type_short_name %>" title="close" onclick="jumpto(<%= FullShipinfos.str_coordinates(@shipinfos) %>, 18)">
    ...
    <article id="shipinfos-dimensions">
      <table>
        <tr>
          ...
          <td>
            <svg id="boat-repr" class="ship-icon <%= @shipinfos.type_short_name %>">
